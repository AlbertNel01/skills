...
When rule 1 or 2 fires with a check unfinished, the verdict line names it — `needs revision + incomplete: step 7` — because an unfinished pre-mortem means rule 1 was never tested. A risk is mitigated when a named task in the plan does the mitigation, so rule 1 does not fire for it. If no test goes red when a blocker's or high risk's mitigation is removed, rule 2 fires instead; a test of other behaviour on the same task does not count. A mitigation this check proposes is not the plan's.
